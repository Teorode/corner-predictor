...
class EnhancedFootballCornerPredictor:
    ...
    def create_advanced_temporal_features(self, df):
        ...
        for idx, row in df.iterrows():
            ...
            df.loc[idx, 'expected_total_corners'] = (home_for + away_against + away_for + home_against) / 2
    ...
    def optimize_xgboost_hyperparameters(self, target_type):
        """Optimized hyperparameters for different prediction types"""
    ...
    def train_enhanced_models(self, X, targets):
        """Train multiple optimized models for different predictions"""
        print(f"\n{'='*80}")
        print("TRAINING ENHANCED CORNER PREDICTION MODELS")
        print(f"{'='*80}")
        ...
        feature_selector_reg = SelectKBest(score_func=mutual_info_regression, k=min(25, X.shape[1]))
        ...
        # Train classification models for each corner line
        print(f"\n{'='*60}")
        ...
        for line in self.corner_lines:
            ...
            feature_selector_clf = SelectKBest(score_func=mutual_info_classif, k=min(20, X.shape[1]))
            ...
            self.models[target_name] = model_clf
            
            # Feature importance
            self.feature_importance[target_name] = dict(zip(selected_features_clf, model_clf.feature_importances_))
            
            # Evaluation
            split_idx = int(len(X_selected_clf) * 0.85)
            X_train_clf, X_test_clf = X_selected_clf[:split_idx], X_selected_clf[split_idx:]
            y_train_clf, y_test_clf = y_clf[:split_idx], y_clf[split_idx:]
            
            final_model_clf = xgb.XGBClassifier(**xgb_params_clf)
            final_model_clf.fit(X_train_clf, y_train_clf)
            
            y_pred_clf = final_model_clf.predict(X_test_clf)
            y_pred_proba_clf = final_model_clf.predict_proba(X_test_clf)[:, 1]
            
            accuracy = accuracy_score(y_test_clf, y_pred_clf)
            log_loss_score = log_loss(y_test_clf, y_pred_proba_clf)
            
            results[target_name] = {'accuracy': accuracy, 'log_loss': log_loss_score}
            
            print(f"Over {line} - Final Accuracy: {accuracy:.3f}, Log Loss: {log_loss_score:.3f}")
        
        return results
    
    def predict_all_corner_lines(self, match_data):
        """Predict corners for all lines with confidence intervals"""
        if not self.models:
        ...
        for line in self.corner_lines:
            ...
            if target_name in self.models:
                ...
                prob = self.models[target_name].predict_proba(features_clf.reshape(1, -1))[0]
                prediction = self.models[target_name].predict(features_clf.reshape(1, -1))[0]
                ...
                predictions['corner_lines'][line] = {
                    ...
                    'edge': abs(over_prob - 0.5) * 2  # Edge calculation
                }
    ...
    def generate_betting_recommendations(self, predictions, match_info=None):
        ...
        print(f"  • Only bet when confidence > 65% and edge > 0.2")
        print(f"  • Consider combining multiple medium-confidence bets")
        print(f"  • Always check actual bookmaker odds before placing bets")
        print(f"  • Manage bankroll - never risk more than 2-5% per bet")
...
def main():
    ...
    try:
        ...
        predictor.display_feature_importance(top_n=15)
        ...
        sample_match = {
            ...
            'btts_percentage_pre_match': 50,
            'over_25_percentage_pre_match': 57,
            'home_corners_avg_6': 5,
            'away_corners_avg_6': 4.17,
            'home_corners_against_avg_6': 3.83,
            'away_corners_against_avg_6': 5.33,
            'expected_total_corners': 9.17,
            'corner_advantage_home': 1.17,
            'corner_advantage_away': -1.16,
            'home_recent_form': 0.4,
